...
### MODEL CODE FROM CRIBB ET AL. (2023) GEOBILOGY ####

#        model function has NOT been edited          #
...
#====== MODEL FUNCTION ======#
model <- function (t,state,parameters,full.output=TRUE){
  with(as.list(c(parameters)),{
    ...
    tran.CH2O.f <- tran.1D(C=CH2O.f,flux.up=F.CH2O.f,v=v.grid,D=Db.grid,VF=svf.grid, dx=grid_data)
    tran.CH2O.s <- tran.1D(C=CH2O.s,flux.up=F.CH2O.s,v=v.grid,D=Db.grid,VF=svf.grid, dx=grid_data)
    tran.FeS    <- tran.1D(C=FeS,   flux.up=F.FeS,   v=v.grid,D=Db.grid,VF=svf.grid, dx=grid_data)

    # Transport terms: Solutes [umol cm-3 pore water yr-1]

    tran.O2   <- tran.1D(C=O2,  C.up=O2.ow,  v=u.grid,D=D.O2.grid,  VF=por.grid, dx=grid_data)
    tran.SO4  <- tran.1D(C=SO4, C.up=SO4.ow, v=u.grid,D=D.SO4.grid, VF=por.grid, dx=grid_data)
    tran.HCO3 <- tran.1D(C=HCO3,C.up=HCO3.ow,v=u.grid,D=D.HCO3.grid,VF=por.grid, dx=grid_data)
    tran.NH4  <- tran.1D(C=NH4, C.up=NH4.ow, v=u.grid,D=D.NH4.grid, VF=por.grid, dx=grid_data)
    tran.HS   <- tran.1D(C=HS,  C.up=HS.ow,  v=u.grid,D=D.HS.grid,  VF=por.grid, dx=grid_data)
    tran.NO3  <- tran.1D(C=NO3, C.up=NO3.ow, v=u.grid,D=D.NO3.grid, VF=por.grid, dx=grid_data)
    ...
    burial.CH2O.f <- tran.1D(C=CH2O.f,flux.up=F.CH2O.f,v=v.grid,D=Db.grid,VF=svf.grid,dx=grid_data)$flux.down
    burial.CH2O.s <- tran.1D(C=CH2O.s,flux.up=F.CH2O.s,v=v.grid,D=Db.grid,VF=svf.grid,dx=grid_data)$flux.down
    burial.FeS    <- tran.1D(C=FeS,flux.up=F.FeS,v=v.grid,D=Db.grid,VF=svf.grid,dx=grid_data)$flux.down
    burial.NH4    <- tran.1D(C=NH4, C.up=NH4.ow, v=u.grid,D=D.NH4.grid, VF=por.grid,dx=grid_data)$flux.down
    burial.HS     <- tran.1D(C=HS,  C.up=HS.ow,  v=u.grid,D=D.HS.grid,  VF=por.grid,dx=grid_data)$flux.down

    # O2 irrigation flux
    int.irr.O2 <- sum(irr.O2*grid_data$dx)
    O2.diff.flux.up <- tran.1D(C=O2, C.up=O2.ow, v=u.grid, D=D.O2.grid, VF=por.grid, dx=grid_data)$flux.up
    ...
    if (!full.output) return(list(c(dCH2O.f,dCH2O.s,dO2,dSO4,dFeS,dHCO3,dNH4,dNO3,dHS),
                                  ...
                                  'FeS.burial'=burial.FeS, 'NH4.burial'=burial.NH4, 'HS.burial'=burial.HS)) else
                                  {
                                    ...
                                    reac.matrix["R1","rate"] <- sum(R1*grid_data$dx)
                                    reac.matrix["R2","rate"] <- sum(R2*grid_data$dx)
                                    reac.matrix["R3","rate"] <- sum(R3*grid_data$dx)
                                    reac.matrix["R4","rate"] <- sum(R4*grid_data$dx)
                                    reac.matrix["R5","rate"] <- sum(R5*grid_data$dx)
                                    reac.matrix["R6","rate"] <- sum(R6*grid_data$dx)
                                    reac.matrix["R7","rate"] <- sum(R7*grid_data$dx)
